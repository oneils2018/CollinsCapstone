...
def object_tracking(x_combined,y_combined,index3,x_hits,y_hits,x_hits0,y_hits0):
    ...
    while len(temp_x_hits) > 0 and index < max_tracking_iterations:
        ...
        if not no_object:
            ...
            if(make_test_data == False):
                ...
                prediction = image_classifier(index,index3)
                plt.text(x_left,y_bottom,prediction)
                os.remove(str(index3)+"zoom"+str(index)+".png")
