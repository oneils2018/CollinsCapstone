...
Description:
    The function will take the filtered data and will display the object center and left, right, top and bottom
    bounds. 
    
    The min_distance is the minimum distance that the algorithm will take before it will lose on trying to find 
    more pixels and will just have the last pixel hit as the bound. 
    
    It will then combine both pixels of 1 and -1.
    
    The x_average=0 and y_average are the coordinated for center of the object. 
    
    It will then go on a loop to calculate the average for x hits and y hits to find the center of object.
    
    This function will find the right bound by first looking at pixel to the right of the center. 
    
    It will note down the number of pixels in a row without a single hit.
    
    It look at the pixels directly to the right of the center.
    
    It will go in a loop to check on the  pixels left to right starting at the center and ending either at the right bound or the edge of the frame.

    It will reset the hit flag and iterate through all of x_hits to see if there's a hot.
    
    If there is a hit, it will update the x coordinates for the right bound and exit the loop.
    else it will keep looping.
    
    If there is hit, it will restart the no_hit counter. The no_hit counter is used to count number of misses in a row 
    
    If there is  no hit, it will increment the misses in a row counter.
    
    If there hasn't been a hit for more than the given minimum distance, give up and exit the loop.
    
    x right is left as the last x coordinate there was hit 
    We then go to the right side 
    
    If the object is being cutoff by the frame, set the right bound to the edge of the frame.
...
Description:
   It will look at the pixels directly to the left of the center
   
   It will go in a loop to check on the  pixels left to right starting at the center and ending either at the left bound or the edge of the frame

   It will reset the hit flag and iterate through all of x_hits to see if there's a hit 
   
   If there is a hit, it will update the x coordinates for the left bound and exit the loop 
   ...
   x right is left as the last x coordinates there was hit 
   
   We then go to the left side 
   
   If the object is being cutoff by the frame, set the left bound to the edge of the frame.
   
   If there was no hit at all, set the bound to 10 pixels to the left of the center
...
Description:
       It look at the pixels directly to the top of the center
       
       It will go in a loop to check on the  pixels left to right starting at the center and ending either at the top bound or the edge of the frame
   
       It will reset the hit flag and iterate through all of x_hits to see if there's a hit 
       
       If there is a hit, it will update the x coordinate for the top bound and exit the loop 
       ...
       We then go to the top side 
       
       If the object is being cutoff by the frame, set the top bound to the edge of the frame.
       
       If there was no hit at all, set the bound to 10 pixels to the top of the center
...
Description:
     It will look at the pixels directly to the bottom of the center
     
     It will go in a loop to check on the  pixels left to right starting at the center and ending either at the left bound or the edge of the frame
 
     It will reset the hit flag and iterate through all of x_hits to see if there's a hot 
     
     If there is a hit, it will update the x coordinate for the right bound and exit the loop 
     ...
     If the object is being cutoff by the frame, set the bottom bound to the edge of the frame.
     
     If there was no hit at all, set the bound to 10 pixels to the bottom of the center
     
     It will Return the coordinates for the center of the object and the right, left, top, and bottom bounds
...
"""
Description: 
    Lastly, it will go through a loop that loops through the video data in increments 
    
    It will call the first filter and return the data to x hits and y hit
    
    It then call the filter10 and return the data for x hit and y hits 
    
    it will plot the data from the first(-1) using skyblue
    
    It will also plot the data from the first filter in pink
    
    tolerance=5 will then set how far the box should be from the actual top, bottom, left and right of the objects
    
    We print the time to console and plot the data.
    
    We plot the points for center of the object, left, right, top and bottom bound and we increment time
